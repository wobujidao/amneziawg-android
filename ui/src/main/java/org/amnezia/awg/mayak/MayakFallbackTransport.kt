    ...
    fun start(fb: Fallback, resolvedIp: String? = null): String? {
        ...
        return try {
            val s = WsUdpShim(
                ...
                // 🔴 КТО ЗАКРЫЛ СОКЕТ. Строка «оборван» одинаково писалась и когда мост честно
                // закрыл поток, и когда его снесла платформа телефона, — и разбор обрыва, который
                // на одном аппарате повторялся 4 раза из 4 (21-08), упёрся именно в это молчание.
                // Тег ТОТ ЖЕ, что у соседей: DiagCollector забирает строки по нему.
                onDown = { e ->
                    Log.i(
                        TAG,
                        if (e == null) "канал закрыт мостом (конец потока)"
                        else "канал оборван на нашей стороне: ${e.javaClass.simpleName}: ${e.message}",
                    )
                },
            )
            s.start()
