// Замер задержки до сервера ЖИВОГО туннеля («Пинг: N мс» под статусом + детектор «Защищено, а трафика
// нет») — НАСТОЯЩИЙ ICMP через СИСТЕМНЫЙ ping (как `ping` на ПК). Клиентский пинг НАПРОТИВ СТРАН в
// списке снят 15-08 (директива владельца) — этот модуль теперь пингует только текущее подключение.
