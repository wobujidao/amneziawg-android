...
class MayakSession(
    ...
    /**
     * Одноразовая ссылка «открыть кабинет уже вошедшим». Возвращает готовый адрес или пусто, если
     * ядро её не дало (тогда звонящий открывает кабинет обычным адресом — см. [MayakCabinet]).
     *
     * Пусто, а не исключение: у этого запроса нет «плохого» исхода, ради которого стоило бы
     * останавливать человека на пути к оплате.
     */
    suspend fun cabinetLink(backend: MayakBackend): String =
        runCatching { backend.cabinetLink(requireToken()).url }.getOrDefault("")
