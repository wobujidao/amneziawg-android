...
class MayakTelemetryWorker(
    ...
    private fun buildBeacon(context: Context): TelemetryRequest = TelemetryRequest(
        ...
        // Тумблер «Открывать российские сервисы напрямую» (RU-пресет): без этого поля сервер не видел
        // настройку вообще, а при ВЫКЛ банки и Госуслуги идут через туннель и могут не пустить с
        // загран-IP (2026-08-03). Ядро принимает поле с этой же даты — сервер выкатывается ПЕРВЫМ.
        ruDirectEnabled = MayakPrefs.ruDirect(context),
    )
