...
class MayakTelemetryWorker(
    ...
    private fun buildBeacon(context: Context): TelemetryRequest = TelemetryRequest(
        ...
        // Реально применяется ли системный РФ-пресет (тумблер пресета ВКЛ + активен системный, а не свой
        // — MayakPresets.ruSplitActive): без этого поля сервер не видел настройку вообще, а при ВЫКЛ
        // банки и Госуслуги идут через туннель и могут не пустить с загран-IP (2026-08-03). ⚠️ НЕ
