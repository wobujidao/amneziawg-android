...
class MayakActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Имя канала уведомлений система замораживает на момент его создания: сменил человек язык
        // приложения или переписали мы текст в новой версии — до него это не доедет само. Освежаем
        // при открытии (только если канал уже есть — заводится он при первом подключении).
        MayakNotification.refreshChannel(this)
    ...
    override fun onResume() {
        ...
        if (isHomeShown) {
            ...
            syncMessages()
            // 🔴 И СРОК ДОСТУПА. Он меняется НЕ в приложении: человек привязывает почту или платит в
    ...
    private fun doLinkCheck(d: Direction) {
        ...
        // 🔴 ЗАПОМИНАЕМ, БЫЛА ЛИ СВЯЗЬ. Проверка гасит туннель после каждой ступени и раньше
        // ЗАКАНЧИВАЛА В ОПУЩЕННОМ состоянии: человек нажимал «проверить» и оставался без связи,
        // ничего об этом не прочитав (найдено на проверке 0.5.44 — экран показывал «Не защищено»,
        // и подключение само не возвращалось). Диалог обещает выключить связь НА ВРЕМЯ проверки,
        // а не насовсем, — значит по её окончании линию надо вернуть туда, где взяли.
        val былаСвязь = tunnel.isUp()
        ...
        linkCheckJob = lifecycleScope.launch {
            ...
            try {
                ...
                linkCheckJob = null
                renderState(ConnState.DISCONNECTED)
                // Возвращаем связь, если она была до проверки. ЗДЕСЬ, а не в теле try: вернуть её
                // надо и когда проверка провалилась на полпути, и когда человек её отменил, —
                // «выключено на время проверки» обязано означать именно это.
                if (былаСвязь) connectTo(d)
            }
        }
    }
    ...
    private suspend fun bringUpUdp(
        ...
        // 🔴 НЕ ЖДАТЬ ПОВТОРА ДВИЖКА, КОГДА КОНФИГ ТОЛЬКО ЧТО ВЫДАН (21-08).
        //
        // Что происходило. Пир доезжает до ноды почти мгновенно (ядро отвечает агенту в момент
        // изменения, длинный опрос), но первую инициацию телефон отправляет ровно в ту же секунду,
        // что и запрос конфига, — и проигрывает эту гонку на доли секунды. Пира на ноде ещё нет,
        // AmneziaWG молча роняет пакет, а СЛЕДУЮЩУЮ попытку движок делает через ~5 с (REKEY_TIMEOUT,
        // не наша настройка). Диаг-лог владельца #87 показывает это дословно: инициация в 15:27:25.151
        // — тишина, повтор в 15:27:30.166 — ответ через 30 мс. Пять секунд из шести с половиной.
        //
        // Лечение: если конфиг СВЕЖИЙ (значит пир мог ещё ехать) и рукопожатия нет через
        // REINIT_AFTER_MS, переподнимаем туннель — это заставляет движок отправить инициацию заново,
        // не дожидаясь его собственного таймера. Стоит переподъём десятки миллисекунд.
        //
        // Ровно ОДИН раз за ступень: если и вторая инициация ушла в тишину, дело не в гонке, и
        // долбить туннель перезапусками бессмысленно — дальше работают обычные пороги лестницы.
        // Порог 1,2 с выбран с запасом: нормальное рукопожатие укладывается в 250 мс (те же логи).
        val reinit = if (peerSyncSlackMs > 0) lifecycleScope.launch {
            delay(REINIT_AFTER_MS)
            if (!withContext(Dispatchers.IO) { tunnel.hasHandshake() }) {
                android.util.Log.i(PROBE_TAG, "рукопожатия нет за ${REINIT_AFTER_MS}мс, а конфиг свежий — переподнимаю туннель, чтобы не ждать повтора движка (~5 с)")
                runCatching { tunnel.up(prepareConf(conf)) }
            }
        } else null
        ...
        return try {
            val ip = if (hasNextRung) probeUntilThreshold(peerSyncSlackMs) else probeWithRetry()
            // 🔎 ТИХАЯ ПРОВЕРКА ПРОБЫ САМОЙ СЕБЯ. На ПРЯМОЙ ступени адрес выхода обязан совпадать с
            // адресом, на который эта ступень и вела: мы уходим в интернет ровно через ту ноду.
            // Не совпал — почти наверняка проба ушла МИМО туннеля (замер 21-08 на эмуляторе: раз из
            // пяти `проба ядро` возвращала публичный адрес самой машины). Вреда в тот раз не было —
            // туннель работал, — но в плохом случае это способ подтвердить выход, которого нет.
            //
            // Пока только ПИШЕМ В ЛОГ и ничего не меняем в поведении: судить по этому признаку
            // опасно, пока не видно по живым отчётам, как часто он срабатывает и не бывает ли
            // ложным (у транзита и моста адреса законно разные — потому и проверяем только прямую).
            //
            // ⚠️ Это ДОГАДКА ПО ЦИФРАМ, и она слабее, чем кажется: совпадение адреса выхода с
            // адресом ступени — не доказательство, а совпадение. Настоящий ответ с 21-08 даёт САМО
            // ЯДРО (поле `ours` в /v1/egress-check: «этот адрес я узнаю как выход своей ноды»), и
            // печатает его HttpEgressProbe строкой рядом. Эта проверка оставлена как второй,
            // независимый взгляд — и потому, что старые ядра поля не присылают.
            if (ip != null && route == GoTunnel.ROUTE_DIRECT && !serverHost.isNullOrBlank() &&
                serverHost.none { it.isLetter() } && ip != serverHost
            ) {
                android.util.Log.i(PROBE_TAG, "выход подтверждён адресом $ip, а прямая ступень вела на $serverHost — проба могла уйти мимо туннеля")
            }
            ip
        } finally {
            reinit?.cancel()
    ...
    private suspend fun probeUntilThreshold(peerSyncSlackMs: Long = 0L): String? {
        ...
        try {
            while (true) {
                ...
                // ⏳ НЕ СТРЕЛЯТЬ В ТУННЕЛЬ, КОТОРЫЙ ЕЩЁ НЕ ОЖИЛ. Проба, запущенная до рукопожатия,
                // не имеет ни единого шанса, зато её собственный таймаут (4 с) идёт полным ходом —
                // и когда рукопожатие опаздывает (потерян первый пакет, пир едет на ноду), человек
                // платит эти 4 с ни за что. Замер на эмуляторе 21-08: рукопожатие на 5263 мс, проба
                // умерла на 4019 мс, «Защищено» пришло только на 5751 мс — вся разница здесь.
                // Ждём рукопожатия, но НЕ бесконечно: если движок о нём почему-то не рассказывает,
                // короткая отсрочка дешевле, чем не проверить выход вовсе.
                val туннельГотов = handshakeAt != null || elapsed >= FIRST_PROBE_GRACE_MS
                if (running == null && туннельГотов && elapsed >= nextTryAt) running = probeScope.async { probe.externalIp() }
    ...
    companion object {
        ...
        /** Через сколько переподнять туннель, если конфиг свежий, а рукопожатия всё нет (см.
         *  bringUpUdp). 1,2 с — с запасом больше обычных 250 мс и заведомо меньше повтора движка
         *  (~5 с), то есть срабатывает ровно на «пир не успел доехать к первой инициации». */
        private const val REINIT_AFTER_MS = 1_200L

        /** Сколько ждать рукопожатия, прежде чем пробовать выход ВСЛЕПУЮ. Обычное рукопожатие
         *  укладывается в 30–300 мс, так что в норме эта отсрочка не наступает вовсе; нужна она на
         *  случай, когда движок о рукопожатии молчит, — тогда проверить выход всё-таки надо. */
        private const val FIRST_PROBE_GRACE_MS = 800L
