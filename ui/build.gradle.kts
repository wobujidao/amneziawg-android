...
android {
    ...
    androidResources {
        ...
        // 🔴 В сборку кладём ТОЛЬКО те языки, на которых приложение говорит целиком (см.
        // MayakLanguages.LANGS). Без этого фильтра в APK едут переводы androidx/Material на три
        // десятка языков, и телефон, настроенный, скажем, на казахский, показывал бы наши экраны
        // по-английски, а библиотечные подписи — по-казахски (замерено 16-08: «Show password» →
        // «Құпия сөзді көрсету» на экране входа). Один экран на двух языках читается как поломка.
        // Заодно APK худеет: тридцать локалей библиотек — это мегабайты ресурсов.
        // ⚠️ Появится настоящий перевод — добавлять тег И сюда, И в MayakLanguages.LANGS.
        localeFilters += listOf("en", "ru")
    }
    // Двойной замок на GPL-бинари amneziawg-tools. Собирать их мы перестали (tunnel/build.gradle.kts,
    // targets только libwg-go.so), но одного этого мало: cmake кладёт .so в свой каталог сборки, и
    // ранее собранные файлы там ОСТАЮТСЯ — при инкрементальной сборке (а релиз на kz-serv собирается
    // именно так) они спокойно доезжают в APK. Проверено 17-08: после правки targets пересборка дала
    // APK ТОГО ЖЕ РАЗМЕРА со всеми libwg.so на месте. Здесь запрет действует в момент упаковки, то
    // есть не зависит от чистоты дерева сборки.
    // ⚠️ Понадобится путь «ядерный модуль + root» — снимать ОБА замка, иначе будет «файл не найден».
    packaging {
        jniLibs {
            excludes += "**/libwg.so"
            excludes += "**/libwg-quick.so"
        }
    }
    // 🔴 ЯЗЫКИ — ОДНИМ КУСКОМ, БЕЗ РАЗДЕЛЕНИЯ (найдено 19-08 живой проверкой бандла на эмуляторе).
    // По умолчанию Play режет бандл в том числе ПО ЯЗЫКАМ и присылает телефону только его язык. У нас
    // русские строки уезжали отдельным куском на 110 КБ, и на телефоне с английским языком системы их
    // не было ВООБЩЕ. Человек открывал «Язык» → «Русский», система выбор запоминала, а интерфейс
    // оставался английским: ресурсов для ru на устройстве нет, resolver падает на язык по умолчанию.
    // Кнопка есть, нажимается, ничего не происходит — худший вид поломки.
    //
    // Правильных путей два: тянуть недостающий язык через Play Feature Delivery (SplitInstallManager,
    // ещё одна зависимость и код, который вне Play не работает) — или не резать вовсе. У приложения
    // ДВА языка и цена вопроса 110 КБ на 19 МБ, поэтому не режем.
    //
    // ⚠️ Проверяется это ТОЛЬКО на бандле, распакованном bundletool'ом: в APK с сайта все языки и так
    // лежат вместе, и там дефекта не видно никогда (тот же класс ошибки, что 13-08).
    bundle {
        language {
            enableSplit = false
        }
