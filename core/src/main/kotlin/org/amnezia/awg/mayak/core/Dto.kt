...
data class Obfuscation(
    ...
    // Случайная длина пакетов рукопожатия (AWG 3.1, свойство линии — миграция 0146). У WireGuard
    // длина рукопожатия постоянная (148 и 92 байта), и это самый дешёвый признак для DPI; движок
    // 3.1 дописывает случайный хвост и постоянство убирает.
    // 🔴 ПАРНАЯ настройка: включённая на сервере, она обязана быть включена и здесь, иначе движок
    // отбросит ответ сервера как чужой пакет. Обратное тоже верно — поэтому по умолчанию false,
    // и решение принимает сервер, а не сборка: линию без флага мы менять не смеем.
    // Сборки старее 0.5.12 работают на движке 3.0 и флага не знают вовсе; ядро не пускает их на
    // такие линии (отказ app_too_old_for_line).
    @SerialName("random_trailers") val randomTrailers: Boolean = false,
)
...
data class AppVersionInfo(
    ...
    /**
     * «Что нового» по-английски. Поле НЕОБЯЗАТЕЛЬНОЕ: у старых выкладок его нет, и тогда всем
     * достаётся русский текст — ровно как было.
     *
     * Зачем завели (16-08): окно обновления собирается из двух источников — рамка («Доступно
     * обновление», кнопки) живёт в ресурсах приложения и переводится сама, а список изменений
     * приезжает с сервера ОДНОЙ строкой. На английском телефоне выходила полукровка: заголовок
     * английский, а внутри стена русского текста. Замечено живой проверкой самообновления на
     * эмуляторе, а не тестом.
     */
    @SerialName("changelog_en") val changelogEn: String = "",
) {
    ...
    /**
     * «Что нового» на языке человека. [bucket] — та же «корзина языка», по которой подписывается кэш
     * названий стран ([MayakBackend.namesLanguageBucket]), чтобы язык списка изменений и язык
     * интерфейса не разъезжались. Английского текста нет → отдаём русский: молчать хуже.
     */
    fun changelogFor(bucket: String): String =
        if (bucket != "ru" && changelogEn.isNotBlank()) changelogEn else changelog
}
 ...
 */
/**
 * Одна строка ЖУРНАЛА ПОДКЛЮЧЕНИЙ (POST /v1/client/connect-log, миграция ядра 0167).
 *
 * Зачем отдельно от недельного бикона: бикон копит счётчики с момента установки и приезжает раз в
 * неделю — по нему видно погоду в среднем, но нельзя посмотреть КОНКРЕТНОЕ подключение. А
 * спрашивают всегда именно так: «вчера вечером не подключалось».
 *
 * Шлётся на КАЖДУЮ попытку, включая удачные (решение владельца 21-08): без фона удачных сравнивать
 * не с чем — «у него всегда мост» видно только на фоне «у остальных прямая».
 *
 * Кто именно подключался, в теле НЕ передаётся: ядро берёт человека из токена.
 * Только техника — ни адресов, ни содержимого трафика.
 */
@Serializable
data class ConnectLogRequest(
    /** Код страны (nl/pl/ru), а НЕ имя: имя мы показываем на языке телефона, и одна страна
     *  расползалась в панели на «Нидерланды» и «Netherlands». */
    @SerialName("direction_code") val directionCode: String,
    /** Ступень, довёзшая до сети: direct | relay | fallback | none (LadderTelemetry.ROUTE_*). */
    val step: String,
    val ok: Boolean,
...
/** Чем кончился поход за списком. «Не менялось» и «не смог» — РАЗНЫЕ исходы: в кэше в обоих случаях
 *  остаётся прежнее, но в логе это должно читаться по-разному, иначе тишину не отличить от поломки. */
sealed class RuDirectFetch {
    /** Ядро ответило 304: у клиента уже свежая версия, тело не передавалось. */
    object NotModified : RuDirectFetch()

    /** Пришёл новый список. */
    class Ok(val list: RuDirectList) : RuDirectFetch()

    /** Сеть, ядро или разбор ответа. Кэш/ассет остаются прежними. */
    class Failed(val cause: Throwable) : RuDirectFetch()
}
...
data class AccountStatus(
    ...
    /**
     * Доступ ПРОБНЫЙ, а не оплаченный (ядро: `plans.code = 'trial'`).
     *
     * Нужно ровно для одного — не кричать красным на новичка. Строка «осталось N дней» красится
     * тревожно от трёх дней и меньше, а пробный без почты длится ровно три дня (решение владельца
     * 16-08), то есть первый экран был красным с первой секунды. Пробный кончается ПО ЗАМЫСЛУ,
     * и это не тревога, пока не наступил последний день.
     *
     * Умолчание false: старое ядро поля не присылает, и тогда всё работает как раньше.
     */
 ...
 *
 * Зачем она есть: приложение авторизовано, а кнопки «Выбрать тариф» и «Открыть кабинет» открывали
 * кабинет голым адресом — то есть формой входа, ровно в тот момент, когда человек готов заплатить.
 * У учёток без почты забытый пароль означает конец пути, восстанавливать нечем.
 *
 * Ссылка живёт минуты и сгорает при первом открытии — хранить её и открывать позже нельзя.
 */
@Serializable
data class CabinetLink(
    val url: String = "",
