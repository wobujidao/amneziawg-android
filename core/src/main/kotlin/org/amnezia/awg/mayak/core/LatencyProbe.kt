...
//    «Авто» (orderForAutoWithHistory в ConnectHistory.kt): свежий замер поднимает выход выше; нет — порядок сервера.
...
/*
 * Порядок «Авто» переехал в ConnectHistory.kt (orderForAutoWithHistory) вместе со вторым шагом —
 * учётом собственного опыта подключения. Здесь функции больше нет НАРОЧНО: две сортировки для
 * одного списка неизбежно разъедутся, а звать в бою надо ровно одну.
 */
