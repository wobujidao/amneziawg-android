...
class MayakBackend(
    ...
    private fun doRequest(
        ...
        try {
            ...
            // 🔢 НОМЕР ЗАПРОСА. Разбор жалобы человека до сих пор был сведением ДВУХ журналов ПО
            // ЧАСАМ: телефон пишет своё, ядро своё, и «это тот самый запрос или соседний» решалось
            // на глаз. Теперь у каждого обращения есть короткий общий номер: он уходит в заголовке,
            // ядро пишет его в свою строку журнала (поле `rid`), а телефон — в свою.
            // Ядро без заголовка ведёт себя как раньше и выдаёт номер само, так что старые ядра
            // эта правка не трогает.
            ...
            if (code !in 200..299) {
                // Номер запроса в журнал телефона пишем ТОЛЬКО при отказе: на успехе он никому не
                // нужен, а строка на каждый запрос утопила бы полезное в шуме. При отказе он —
                // единственное, что позволяет найти ЭТОТ ЖЕ запрос в журнале ядра за секунду.
                MayakDiagSink.log("обращение $rid: $method ${hostOf(url)} → HTTP $code")
...
/**
 * newRequestId — короткий номер обращения: шесть символов из букв и цифр.
 *
...
/**
 * hostOf — только хост из адреса, для строки журнала.
 *
 * Путь и строку запроса НЕ пишем намеренно: именно в них ездят одноразовые коды и токены перехода
 * в кабинет — то же правило, что у журнала обращений на ядре.
 */
internal fun hostOf(url: String): String = runCatching { URL(url).host }.getOrDefault("?")
