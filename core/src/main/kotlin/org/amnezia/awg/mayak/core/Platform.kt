...
/**
 * Стабильный идентификатор «железа» (HWID): переживает ПЕРЕУСТАНОВКУ приложения, чтобы ядро узнавало
 * то же физическое устройство и не считало переустановку новым устройством (иначе ложно срабатывает
 * лимит устройств). Android-реализация — SHA-256(ANDROID_ID + соль) — НЕ сырой ANDROID_ID (приватность).
 */
interface HwidProvider {
    /** Стабильный hwid (hex-строка). Один и тот же на одном устройстве после переустановки. */
    fun hwid(): String
}
